...
namespace Puresharp
{
    static internal partial class Data
    {
        ...
        {
            static public implicit operator T[] (Data.Enumerable<T> enumerable)
            {
            ...
            public IEnumerator<T> GetEnumerator()
            {
                if (this.m_Activated)
                {
                    if (this.m_List == null) { return this.m_Enumerable.GetEnumerator(); }
                    else { return this.m_List.GetEnumerator(); }
                }
                return new Enumerable<T>.Enumerator(this);
            }
            ...
            public int IndexOf(T item)
            {
                if (this.m_Activated)
                {
                    if (this.m_List == null)
                    {
                        using (var _enumerator = this.m_Enumerable.GetEnumerator())
                        {
                            var _index = 0;
                            while (_enumerator.MoveNext())
                            {
                                if (object.Equals(_enumerator.Current, item)) { return _index; }
                                _index++;
                            }
                        }
                        return -1;
                    }
                    else { return this.m_List.IndexOf(item); }
                }
                using (var _enumerator = new Enumerable<T>.Enumerator(this))
                {
                    var _index = 0;
                    while (_enumerator.MoveNext())
                    {
